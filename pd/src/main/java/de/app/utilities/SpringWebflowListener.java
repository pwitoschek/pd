...
public class SpringWebflowListener extends FlowExecutionListenerAdapter {
	 ...
	 * org.springframework.webflow.definition.StateDefinition)
	 */
	// Methode zum ermitteln des aktuellen Ausf�hrungsschrittes
	...
	public void viewRendering(RequestContext context, View view,
		...
		// Navigation �ber den Browser die Navigation anpassen zu k�nnen
		...
		Navigation nav = (Navigation) context.getConversationScope().get(
				"navigation");
		// K�nnte sein, dass es der Einstieg ist, weshalb die Navigation noch
		// nicht in der Session als Objekt gespeichert ist.
		// Deshalb hier an der Stelle eine null-�berpr�fung
		if (nav != null) {
	...
	public void stateEntered(final RequestContext context,
		...
		if (newState != null
			...
			// Falls ein Modelattribut �bergeben worden ist, muss es in die
			// Session gespeichert werden um sp�ter
			// festzustellen ob ein Sprung nach "vorne" in der Navigation
			// erlaubt ist

			// Aktuell wird im Formular das Hidden-Input Feld unter dem Key
			// "formModel" �bergeben um den Namen des Models des Formulars zu
			// kennen
			// Mit dem Namen l�sst sich �ber den Flow Scope und dessen
			...
			// zu vergleichen
			// je nachdem ob etwas am Model ge�ndert worden ist, darf der Kunde
			// dann an die gew�nschte Stelle navigieren.
			...
			// Navigation deren w�rde nur genau die States aus dem Subflow
			// enthalten
			...
			if (!(newState instanceof SubflowState)) {
				Navigation nav = (Navigation) context.getConversationScope()
						.get("navigation");
				// K�nnte sein, dass es der Einstieg ist, weshalb die Navigation
	...
	public void sessionStarting(RequestContext context, FlowSession session,
		...
		if (definition instanceof Flow && session.getParent() == null) {
			...
			for (int i = 0; i < stateIds.length; i++) {
				...
				// View-States und Substates m�ssen in die Navigation �bernommen
				// werden
				...
				if (isViewState) {
					...
					item.setDisplayName(new PropertyUtilities().getFlowProperties(definition.getId() , stateIds[i]));
					items.add(item);
				}
			}
